...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const isRetreiveBtnClicked = false;
  ...
  // Fetch functions
  const validateAuth = () => fetch('/obs-form/auth')
    .then((response) => {
    ...
    .catch((error) => { });
  const postDoctorInfo = (doctorEntry) => fetch('/postDoctorInfo', {
    method: 'POST',
  ...
  const postStudentInfo = (studentEntry) => fetch('/postStudentInfo', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const postFormInfo = (formEntry) => fetch('/postFormInfo', {
    method: 'POST',
    headers: {
  ...
  const postAcknowledge = (formEntry) => fetch('/postAcknowledge', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  const updateFormStatus = (studentId) => fetch(`/updateFormStatus?studentId=${studentId}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  const sendEmail = (emailEntry) => fetch('/send-email', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  const getSchools = () => fetch('/getSchools')
    .then((response) => {
      if (!response.ok) {
  ...
  const uploadSignature = (data) => fetch('/uploadSign', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  const checkStudentDuplication = (studentNRIC) => fetch('/checkStudentNRIC', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ studentNRIC }),
  })
    .then((response) => {
  ...
  const getStudentRegistrationInfo = (registrationEntry) => fetch('/getStudentRegistrationInfo', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const waitModalResponse = (deletionStudentEntry) => new Promise((resolve, reject) => {
  ...
  availabilityBtn.addEventListener('click', (event) => {
    ...
    if (validities.isDoctorMCRValid) {
      ...
      fetch('/checkDoctorMCR', {
        ...
        .then((data) => {
          const {
            doctorMCR, nameOfDoctor, signature, nameOfClinic, clinicAddress, contactNo,
          } = data[0];
        ...
        data.forEach((school, index) => {
          ...
          if (isFirstIteration) {
            ...
            schoolSearch.addEventListener('input', (event) => {
              const schoolSearchValue = event.target.value;
              const matchingSchools = schools.filter((school) => school.toLowerCase().includes(schoolSearchValue.toLowerCase()));
              const lis = schoolDropDownMenu.querySelectorAll('li');
              lis.forEach((li) => {
                li.remove();
              });
          ...
          if (isDoctorNew === true) {
            ...
            uploadSignature(data)
              ...
              .then((data) => {
                ...
                const scrollableDiv = document.getElementById('formDiv');
