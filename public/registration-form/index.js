window.addEventListener('DOMContentLoaded', () => {
  ...
  const displayNote = (callingEl, note, type) => {
    ...
    if (note === 'specialist memo') {
      message = 'A specialist memo is needed for further assessment. Complete this registration and get in touch with the Teacher Coordinator.';
    } else if (note === 'physiotherapist memo') {
      message = 'A specialist/physiotherapist memo is needed for further assessment. Complete this registration and get in touch with the Teacher Coordinator.';
    } else if (note === 'treating memo') {
      message = 'A specialist/treating doctor memo is needed for further assessment. Complete this registration and get in touch with the Teacher Coordinator.';
    } else if (note === 'unable to enrol') {
      message = 'OBS is unable to enrol the Applicant with the stated condition. Complete this registration and get in touch with the Teacher Coordinator for advice.';
    }
    ...
    if (insertNoteTemplateDiv) {
      ...
      if (type) {
        ...
        content.querySelector('h3').textContent = `Note: ${message}`;
  ...
  const validateApplicantSection = () => {
    ...
    const optionalFields = ['applicantDiet'];
    const applicantSectionValues = {};
  ...
  const validateHealthSection = () => {
    ...
    if (healthSectionValues.otherConditionStatus != 1) {
      optionalFields.push(
        ...
        'otherDiagnosedDate',
        'otherPhysicalEngagement',
        'otherTriggerFactor',
        'otherMeasures',
        'otherFollowup',
        'otherFocusAbility',
        'otherUnderstandAbility',
        'otherHelp',
        'otherMedication',
      );
  ...
  const validateDeclaration = () => {
    ...
    const declarationSectionInputs = {
    ...
    const optionalFields = ['disagreeEvents', 'disagreeMedia'];
  ...
  document.getElementById('submit-button').onclick = (e) => {
    e.preventDefault();
    const parentValidation = validateParentSection();
    const applicantValidation = validateApplicantSection();
    const healthValidation = validateHealthSection();
