window.addEventListener('DOMContentLoaded', () => {
  ...
  const createUser = (newuser) => fetch('/obs-admin/newuser', {
    method: 'POST',
    headers: {
  ...
  // EDIT USER
  const editUser = (user) => fetch('/obs-admin/user', {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  const deleteButtonHandler = (user) => {
    ...
    // DELETE USER
    document.getElementById('confirmation-delete-button').onclick = () => fetch(`/obs-admin/delete/user/${user.email}`, {
      method: 'PUT',
    })
      .then(handleCreateEditResponse)
      .then(() => {
  ...
  const bulkDelete = (users) => {
    ...
    document.getElementById('confirmation-delete-bulk-button').onclick = () => fetch('/obs-admin/delete/user', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const disableButtonHandler = (user, status) => {
    ...
    // DISABLE USER
    document.getElementById('confirmation-disable-button').onclick = () => fetch(`/obs-admin/disable/user/${user.email}/${status}`, {
      method: 'PUT',
    })
      .then(handleCreateEditResponse)
      .then(() => {
  ...
  const bulkDisable = (users) => {
    ...
    // BULK DELETE PERMISSION GROUPS
    document.getElementById('confirmation-disable-bulk-button').onclick = () => fetch('/obs-admin/disable/user', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const enableButtonHandler = (user, status) => {
    // ENABLE USER
    document.getElementById('confirmation-enable-button').onclick = () => fetch(`/obs-admin/disable/user/${user.email}/${status}`, {
      method: 'PUT',
    })
      .then(handleCreateEditResponse)
      .then(() => {
  ...
  const updateCheckedCount = () => {
    ...
    const enable = values.some((i) => document.getElementById(`item-${i}`).getElementsByClassName('dropdown-disable')[0].textContent === 'Enable');
